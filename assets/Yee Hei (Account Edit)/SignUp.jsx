...
const SignUp = (props) => {
    ...
    const onFinish = () => {
        //console.log(form.getFieldsError().some((item) => item.errors.length > 0));
    ...
    const checkEmail = ({getFieldValue})=>({
        validator(_, value){
            ...
            // users.forEach(acc => {
            // for (let key in acc)
            //         if (key === "email" && acc[key].toLowerCase() == value.toLowerCase()){
            //             s = false;
            //             break;
            //         }
            // })
            // return (s ? Promise.resolve() : Promise.reject("Invalid email"));
        }
    })
    const checkName = ({getFieldValue})=>({
        validator(_, value){
            if (value == undefined)
    ...
    return <div className={AccountCss.signUpContainer}>
        <div className={AccountCss.form}>
            <Form form={form} name="form">
                ...
                <Form.Item label="Address" name="address"  rules = {[ ]} hasFeedback>
                        <Input />
                </Form.Item>

                <Form.Item>
                    <Button type="primary" htmlType="submit" onClick={onFinish}> Create Account </Button>
                </Form.Item>
